<template>
  <div>
    ...
    <div class="subheading ml-4 mb-2" v-else>
      ...
      <div
        v-if="addressUpdate.city + addressUpdate.state + addressUpdate.zipcode + addressUpdate.country !== ''"
      >{{ addressUpdate.city + ' ' + addressUpdate.state + ' ' + addressUpdate.zipcode + ' ' + addressUpdate.country}}</div>
    </div>
    <v-dialog
      v-model="dialog"
    ...
    >
      <v-card>
        ...
        <v-card-text>
          <v-text-field v-model="addressUpdate.line" label="Address Line"></v-text-field>
          <v-text-field v-model="addressUpdate.city" label="Town/City"></v-text-field>
          <v-text-field v-model="addressUpdate.state" label="Province/State"></v-text-field>
          <v-text-field v-model="addressUpdate.zipcode" label="Zip Code"></v-text-field>
          <!-- <v-select :items="countries" label="Country" v-model="addressUpdate.country" autocomplete></v-select> -->
        </v-card-text>
        <div>
          <v-card-actions>
            <v-spacer></v-spacer>
...
export default {
  ...
  watch: {
    addressUp: function() {
      if (
        this.addressUpdate.line !== this.address.line ||
