  ...
  <div>
    <v-list-tile>
      <v-list-tile-content>
        <v-list-tile-sub-title class="ma-0 pa-0">
          <v-icon class="mr-1">phone</v-icon>Phone Numbers
        </v-list-tile-sub-title>
      </v-list-tile-content>
      <v-list-tile-action>
        <v-btn icon flat class="pa-0 ma-0" @click="dialog = !dialog">
          <v-icon
            v-if="phoneUpdate.mobile === '' && phoneUpdate.business === '' && phoneUpdate.home === ''"
            color="editicon"
          >add</v-icon>
    ...
    >
      <v-card>
        ...
        <v-card-text>
          <v-text-field v-model="phoneUpdate.mobile" label="Mobile Number"></v-text-field>
          <v-text-field v-model="phoneUpdate.business" label="Business Number"></v-text-field>
          <v-text-field v-model="phoneUpdate.home" label="Home Number"></v-text-field>
        </v-card-text>
        <div>
          <v-card-actions>
            <v-spacer></v-spacer>
...
export default {
  ...
  computed: {
    phoneNumberUp: function() {
      return (
        this.phoneUpdate.mobile +
        this.phoneUpdate.business +
        this.phoneUpdate.home
      )
    },
    phoneNumberProp: function() {
      return (
        this.phone.mobile + ' ' + this.phone.business + ' ' + this.phone.home
      )
    },
    breakpoint() {
  ...
  watch: {
    phoneNumberUp: function() {
      if (
        this.phoneUpdate.mobile !== this.phone.mobile ||
